...
var slp_1 = require("../lib/slp");
var assert_1 = __importDefault(require("assert"));
var bitbox_sdk_1 = require("bitbox-sdk");
var bitbox = new bitbox_sdk_1.BITBOX({ restURL: "https://trest.bitcoin.com/v2/" });
var scriptUnitTestData = require('slp-unit-test-data/script_tests.json');
var slp = new slp_1.Slp(bitbox);
describe('Slp', function () {
    describe('parseSlpOutputScript() -- SLP OP_RETURN Unit Tests', function () {
        scriptUnitTestData.forEach(function (test) {
            it(test.msg, function () {
                var script = Buffer.from(test.script, 'hex');
                var eCode = test.code;
                if (eCode) {
                    assert_1.default.throws(function () { slp.parseSlpOutputScript(script); });
                }
                else {
                    var parsedOutput = slp.parseSlpOutputScript(script);
                    assert_1.default(typeof parsedOutput, 'object');
                }
            });
        });
    });
